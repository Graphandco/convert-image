...
app.post('/api/convert', upload.single('image'), async (req, res) => {
  try {
    ...
    if (!['webp', 'avif', 'jpg', 'png', 'ico'].includes(format)) {
      return res.status(400).json({ error: 'Format invalide. Choisissez webp, avif, jpg, png ou ico.' });
    }
    ...
    const validMaxWidth = Number.isInteger(maxWidth) && maxWidth > 0 && maxWidth <= 8000;
    ...
    if (format === 'ico') {
      ...
      const tmpPath = path.join(os.tmpdir(), `convert-ico-${Date.now()}-${Math.random().toString(36).slice(2)}.ico`);
      await ico.sharpsToIco([pipeline], tmpPath, { sizes: [256, 128, 64, 48, 32, 24, 16] });
    ...
    const mime = { webp: 'image/webp', avif: 'image/avif', jpg: 'image/jpeg', png: 'image/png' }[format];
